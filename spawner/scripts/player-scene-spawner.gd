...
func _enter_tree() -> void:
  add_to_group(Z_Path.PLAYER_SCENE_SPAWNER_GROUP)
...
func spawn():
  ...
  if container:
    ...
    if not container.is_inside_tree():
      __z.layer.layer_named(layer).add_child(s)
  else:
    __z.layer.layer_named(layer).add_child(s)
...
func _unhandled_input(event: InputEvent) -> void:
  ...
  if Z_State.is_dying(): return
  if not Z_State.is_game(): return
  if event.is_action_pressed(spawn_action_name):
    __z.audio.director.play_pitched(__z.audio.director.sfx_respawn_active, 1, true, false)
    spawn()
