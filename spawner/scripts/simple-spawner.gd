...
func spawn():
	...
	var target_container : Node = container
	if not target_container and (not __z.layer or not __z.layer.is_node_ready()):
		await __z.bus.sig_layer_managed_layers_ready
		target_container = __z.layer.layer_named(layer)
