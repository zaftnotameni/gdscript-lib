...
@export var layer : Z_Layers.LAYERS = Z_Layers.LAYERS.background
...
func at_layer(target:=Z_Layers.LAYERS.background) -> Z_Primitive_BackgroundColorWithRect:
  layer = target
  return self
...
func add_to_tree() -> Z_Primitive_BackgroundColorWithRect:
  ...
  __z.layer.layer_named(layer).add_child(self)
  return self
