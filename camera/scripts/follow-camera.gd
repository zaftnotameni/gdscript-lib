...
func update_parallax(c:Camera2D):
  var nodes := get_tree().get_nodes_in_group(Z_Path.MAIN_CAMERA_PARALLAX_GROUP)
  for n in nodes:
    if 'material' in n:
...
func _ready() -> void:
  ...
  add_child(shake)
  if follows_player:
    if not target_node: target_node = Z_Global.player
    __z.bus.sig_global_player_set.connect(on_player_global_changed)
...
func _enter_tree() -> void:
  ...
  add_to_group(Z_Path.MAIN_CAMERA_GROUP)
  Z_Global.register_camera(self)

func _exit_tree() -> void:
  print_verbose('follow camera exit tree')
  if Z_Global.camera == self:
    Z_Global.register_camera(null)
