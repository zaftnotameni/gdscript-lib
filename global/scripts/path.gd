class_name Z_Path extends Node
...
static func group_all_nodes(g:StringName=&"Group Name") -> Array[Node]:
  return Z_Util.scene_tree().get_nodes_in_group(g)
...
static func group_main_camera_maybe_first_node(g:=MAIN_CAMERA_GROUP) -> Camera2D:
  return Z_Util.scene_tree().get_first_node_in_group(g)
...
static func await_for_first_node_in_group(g:=PLAYER_CHARACTER_GROUP,max_attempts:=10) -> Node:
  ...
  await Z_Util.scene_tree().process_frame
