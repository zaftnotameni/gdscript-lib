class_name Zaft_Gun extends Node2D
...
@export var pool_layer : Zaft_Autoload_Layers.LAYERS
...
func _ready() -> void:
  ...
  if not pool.is_inside_tree():
    assert(pool_layer != Zaft_Autoload_Layers.LAYERS.invalid, 'must provide a valid layer %s' % get_path())
